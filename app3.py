...
def update_data(query):
    ...
    try:
        execute_query(query)
        return json.dumps({"isSuccess": True, "message": "Update successful"})
    except Exception as err:
        return json.dumps({"isSuccess": False, "error": str(err)})
...
def handle_output(query, key=False, file_path=None, is_stored_procedure=False, is_update=False):
    """Handle the output depending on the key and file path."""
    try:
        if is_update:
            # Call update_data for executing update queries
            response = update_data(query)
            print(response)
        elif key:
            results = fetch_data(query, is_stored_procedure=is_stored_procedure)
            print(json.dumps({"isSuccess": True, "results": results}, cls=CustomJSONEncoder))
...
if __name__ == "__main__":
    try:
        ...
        key = sys.argv[5].lower() == 'true' if len(sys.argv) > 5 else False
        file_path = sys.argv[6] if len(sys.argv) > 6 and sys.argv[6].endswith('.json') else None
        is_update = sys.argv[7].lower() == 'true' if len(sys.argv) > 7 else False
        ...
        if len(sys.argv) > 8:
            config['username'] = sys.argv[8]
            config['password'] = sys.argv[9]
        else:
            config['username'] = None
            config['password'] = None
        ...
        if file_path:
            ...
            handle_output(query, key=key, file_path=file_path, is_stored_procedure=is_stored_procedure, is_update=is_update)
